...
{
	QPointF anchor = mapFromParent(m_chart->mapToPosition(this->anchor));
	QRectF rect;
...
void Callout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	Q_UNUSED(option)
		...
		painter->setFont(QFont("Microsoft YaHei", 10, QFont::Normal, true));
	textRect = painter->fontMetrics().boundingRect(QRect(0, 0, 200, 200), Qt::AlignLeft, text);
	...
	painter->drawText(textRect, Qt::AlignJustify, text);
...
void Callout::updateGeometry(int x_offset,int y_offset)
{
	prepareGeometryChange();
	setPos(m_chart->mapToPosition(anchor) + QPoint(x_offset, y_offset));
}

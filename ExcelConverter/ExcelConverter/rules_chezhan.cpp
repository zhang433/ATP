...
bool Rules_CHEZHAN::convert(QList<QPair<FileInfo, ExcelFile> > &excelFiles, QList<QPair<QString, Sheet> > &All_ConvertSheet)
{
    ...
    {
        if (iter->first.fileType == FileType::CHEZHAN)
		{
    ...
    for(auto& file : files)
    {
        for(auto& sheet: file.second)
        {
            for(auto& line: sheet.second)
            {
                Station station;
                station.ID = line[0];
                station.stationName = line[1];
                station.regionID = line[2];
                station.subareaID = line[3];
                station.stationID = line[4];

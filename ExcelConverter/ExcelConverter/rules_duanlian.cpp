...
bool Rules_DUANLIAN::convert(QList<QPair<FileInfo, ExcelFile> > &excelFiles, QList<QPair<QString, Sheet> > &All_ConvertSheet)
{
    ...
    {
        if (iter->first.fileType == FileType::DUANLIAN)
        {
    ...
    for(auto& file : files)
    {
        for(auto& sheet: file.second)
        {
            for(auto& line: sheet.second)
            {
                ...
                brokenLink.lineName = line[0];
                brokenLink.lineType = line[1];
                brokenLink.brokenLinkType = line[2];
                brokenLink.brokenLinkPosSta = line[3];
                brokenLink.brokenLinkPosEnd = line[4];
                brokenLink.brokenLinkLongLen = line[5];
                brokenLink.brokenLinkShortLen = line[6];
